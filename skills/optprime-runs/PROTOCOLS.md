...
**Job attribution (clarified 2026-08-13, owner-approved).** Ownership §1 is enforced at
the **graph** level (`created_by` on every node = the requesting user). The **platform
job** is a separate matter: `remote.submit` authenticates via `cli.client._auth_headers`,
which forwards **only** `ORBITAL_API_KEY` (`X-API-Key`) and **no**
`X-Forwarded-User-Email`, so the platform attributes the k8s Job to `username: curie`
(the service account), not the user. **This is accepted** — the owner wants it, because a
`curie` attribution is a useful signal that a job was auto-generated by this skill. So do
**not** "fix" it or treat `curie` on the Job/pod as an ownership violation. (Consequence
to remember: the §2 quota gate keys on the user's `username` label and therefore does not
see `curie`-attributed `remote.submit` jobs — see §2.)
